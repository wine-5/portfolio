...
ポートフォリオを「レトロ/8bit ピクセル RPG の世界そのもの」に全面リデザインする。
就活・面接官に見せる用途。実際に遊べるゲームではなく「ゲームっぽく見せる演出」。

- 世界観: レトロ/ドット絵(8bit) RPG。ファミコン/ドット方面(近未来 HUD ではない)
- 配色: 制限なし(カラフルな原色ドット OK)
- フォント: 見出し/UI はドット調、本文は「少しドット感があるが読みやすい」フォント
- 起動: PRESS START 風タイトル → 入力待ちせず 1 秒未満で自動遷移
- テーマ切替(昼/夜マップ風 light/dark)と JP/EN i18n は維持
- サウンドなし。ドット素材はフリー(CC0/Kenney 等)を活用
