...
| 順 | セクション | 演出 |
|---|---|---|
| 1 | **Games(絶対主役)** | モンスター図鑑風一覧(No. 付き・NAME ラベル等の見た目のみ。ATK 等のステータス数値化は不採用 = 2026-07-02 決定) |
...
新実装(`src/`)がトップの `index.html`(2026-07-02 に入口を交代済み)。
旧サイトは `old-site.html` + `js/` + `css/` + `assets/` として並存中で、移行完了後に撤去する。
