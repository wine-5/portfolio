...
export class JsonProfileRepository implements ProfileRepository {
  async getProfile(lang: string): Promise<Profile> {
    ...
    // 固定値で返す
    const socialLinks = this.getSocialLinks(lang);
  ...
  private getSocialLinks(lang: string): Record<string, string> {
    return {
  ...
  private getDescription(lang: string): string {
    if (lang === 'en') {
      return 'Game developer pursuing the possibilities of games with Unity & C#. I focus on SOLID design principles and aim to create games that bring joy to players.';
    }
