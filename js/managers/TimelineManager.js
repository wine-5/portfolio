...
class TimelineManager {
    ...
    ensureDataAndDOM() {
        ...
        // データの確認（window.timelineDataから取得）
        if (!this.timelineData || this.timelineData.length === 0) {
            if (window.timelineData && window.timelineData.isLoaded) {
                this.timelineData = window.TIMELINE_DATA;
            } else if (typeof TIMELINE_DATA !== 'undefined') {
    ...
    init() {
        ...
        // DOM要素が存在しない場合は初期化をスキップ（別ページの可能性）
        this.timelineContainer = document.getElementById('timeline-container');
        if (!this.timelineContainer) {
            console.warn('Timeline container not found - skipping TimelineManager initialization');
            return;
        }
        ...
        if (!this.ensureDataAndDOM()) {
            ...
            if (this.initRetryCount < this.maxRetries) {
                console.warn(`TimelineManager: Waiting for data... (${this.initRetryCount}/${this.maxRetries})`);
                setTimeout(() => this.init(), 500);
            } else {
                console.error('TimelineManager: Failed to load data after maximum retries');
            }
            return;
        }
